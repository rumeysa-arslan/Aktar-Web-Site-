...
// Sayfa başlığı (Title etiketi için)
$page_title = "Tüm Ürünler";
$site_adi = "Aktarhane";

// 2. Veritabanından TÜM ürünleri çek (Stokta olanları, A'dan Z'ye sıralı)
try {
    $stmt = $conn->prepare("SELECT * FROM urunler WHERE stok > 0 ORDER BY ad ASC");
    $stmt->execute();
    $tum_urunler = $stmt->fetchAll(PDO::FETCH_ASSOC);

} catch (PDOException $e) {
    echo "Ürünler getirilirken hata oluştu: " . $e->getMessage();
    $tum_urunler = []; // Hata olursa boş dizi
}
?>
...
<header>
    <nav class="main-nav">
        ...
        <ul>
            ...
            <li><a href="urunler.php" class="active">Tüm Ürünler</a></li>
...
<main>
    <div class="container">
        ...
        <h2>Tüm Ürünler (<?php echo count($tum_urunler); ?> çeşit)</h2>
        
        <div class="urun-grid">
            <?php 
            if (count($tum_urunler) > 0):
            ...
            ?>
                ...
                echo "<p>Stokta hiç ürün bulunmamaktadır.</p>";
            endif;
            ?>
        </div> </div> </main>
