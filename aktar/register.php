...
// Zaten giriş yapmış bir kullanıcıyı bu sayfaya yönlendirme
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // --- GÜVENLİK VE DOĞRULAMA (VALIDATION) ---
    // A. Boş alan kontrolü
    ...
    // --- VERİTABANI KONTROLÜ (Hata yoksa) ---
    if (empty($hatalar)) {
        try {
            // E. Kullanıcı adı veya email zaten var mı?
            ...
            if ($stmt->fetch()) {
                ...
                // F. Hata yoksa, GÜVENLİ ŞİFRE OLUŞTURMA (HASHING)
                $sifre_hash = password_hash($sifre, PASSWORD_BCRYPT);

                // G. Veritabanına YENİ KULLANICIYI EKLE
                ...
                // Başarı mesajı ver ve login'e yönlendir
                $basari_mesaji = "Kayıt başarılı! Lütfen giriş yapın.";
                // Kullanıcıyı login sayfasına yönlendir (opsiyonel, aynı sayfada mesaj da gösterilebilir)
                header("Refresh: 2; url=login.php"); // 2 saniye sonra yönlendir
            }
